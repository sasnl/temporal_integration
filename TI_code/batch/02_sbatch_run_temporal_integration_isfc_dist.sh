...
for lines in `cat $params_txt_file`; do
	...
	echo 'echo "SLURM_NODELIST=$SLURM_NODELIST"' >> TI_isfc.sbatch
	echo 'echo "SLURM_CPUS_PER_TASK=$SLURM_CPUS_PER_TASK"' >> TI_isfc.sbatch
	echo 'echo "SLURM_JOB_CPUS_PER_NODE=$SLURM_JOB_CPUS_PER_NODE"' >> TI_isfc.sbatch
	echo 'echo "SLURM_CPUS_ON_NODE=$SLURM_CPUS_ON_NODE"' >> TI_isfc.sbatch
	echo 'echo "SLURM_MEM_PER_NODE=$SLURM_MEM_PER_NODE"' >> TI_isfc.sbatch
	echo 'echo "SLURM_MEM_PER_CPU=$SLURM_MEM_PER_CPU"' >> TI_isfc.sbatch
	# ---- Thread control (export) ----

	echo 'export OMP_NUM_THREADS=${SLURM_CPUS_PER_TASK:-1}' >> TI_isfc.sbatch
	echo 'export OPENBLAS_NUM_THREADS=${SLURM_CPUS_PER_TASK:-1}' >> TI_isfc.sbatch
	echo 'export MKL_NUM_THREADS=${SLURM_CPUS_PER_TASK:-1}' >> TI_isfc.sbatch
	echo 'export NUMEXPR_NUM_THREADS=${SLURM_CPUS_PER_TASK:-1}' >> TI_isfc.sbatch

    echo "bash 01_run_temporal_integration_isfc_dist.sh \
    \"${code_dir}\" \
    \"${params}\" \
    \"${data_dir}\" \
    \"${output_dir_thisrun}\" \
