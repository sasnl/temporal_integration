...
output_dir=$5 #/oak/stanford/groups/menon/projects/daelsaid/2022_speaker_listener/results/post_processed_wholebrain/filtered/06-2025/td/hpf/isc_analysis_1000_permutations_hpc
mem=$6
...
usage() {
    ...
    echo "         <param_file> <code_dir> <p_threshold> <data_dir> <output_dir> <mem_gb>"
    echo ""
    echo "Arguments:"
...
for lines in `cat $params_txt_file`; do
    ...
    \"${p_threshold}\" \
    \"${data_dir}\" \
    \"${output_dir}\"" >> TI_isc.sbatch
    sbatch -p owners,menon -c 16 --mem=${mem}G -t 6:00:00 -o "${output_dir}/temporal_integration_${condition}_${p_threshold}_log.txt" TI_isc.sbatch;
    rm TI_isc.sbatch;
done