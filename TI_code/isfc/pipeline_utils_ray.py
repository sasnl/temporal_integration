...
def compute_isc_chunk(chunk_data, pairwise):
    ...
    if z_isc.ndim == 1:
        z_isc = z_isc[np.newaxis, :]

    return raw_isc, z_isc

def run_isc_computation(data, pairwise=False, chunk_size=30000):
    """
    ...
    # After Step 2, r_chunk should always be 2D: (n_samples, n_chunk_vox)
    ...
    for i, (r_chunk, z_chunk) in enumerate(results):
        ...
        end_idx = min((i + 1) * chunk_size, n_voxels) 
         #DE added
        n_chunk_vox=end_idx-start_idx

        # check for shape expected (n_samples, n_chunk_vox)
        if r_chunk.ndim != 2 or r_chunk.shape[1] != n_chunk_vox:
            raise ValueError(
                f"Chunk shape mismatch at chunk {i}: "
                f"got r_chunk {r_chunk.shape}, expected ({sample_dim}, {n_chunk_vox})"
            )
